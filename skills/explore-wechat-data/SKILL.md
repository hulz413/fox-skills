---
name: explore-wechat-data
description: |
  微信 Mac 4.x 本地数据库全量/增量解析与数字资产库。用于本机微信聊天记录、联系人、群聊、朋友圈、收藏夹、语音/附件索引的密钥提取、全量解密、增量刷新、指定联系人/群聊导出、关系复盘、客户跟进和内容沉淀。触发词：微信解析、微信全量、微信增量、聊天记录、导出聊天、朋友圈解析、收藏夹解析、客户跟进、explore-wechat-data。
---
...
- 密钥和解密配置放在 `~/.config/`；明文库、manifest、增量状态和临时解密文件默认放在专用的 `~/Developer/playground/wechat/wechat-local-vault/` 目录；可读报告放在对应的 exports 或摘要目录。
- 不要把明文数据库复制到其他项目工作区、桌面、网盘目录或聊天回复里；专用微信数据目录只供本 skill 使用。
...
python3 {{SKILL_DIR}}/scripts/vault_cli.py digest-source "群名" --since-last --data-root ~/Developer/playground/wechat/wechat-digests --format text
```
...
2. 如果 `~/.config/explore-wechat-data-keys.json` 已有 key，先用 `--match-only --targets all --reuse-log` 尝试复用。
